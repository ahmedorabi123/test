...
export interface ManualRefundOrderInput extends OrderOption {
...
interface ManualRefundButtonProps {
  onCreated: () => void;
  /**
   * Optional preselected order. When provided the order picker is hidden and
   * the modal opens scoped to this order only (used by OrderDetailPage and the
   * orders drawer).
   */
  order?: ManualRefundOrderInput;
  triggerLabel?: string;
...
export default function ManualRefundButton({
  ...
  const [order, setOrder] = useState<OrderDetail | null>(lockedOrder ?? null);
  ...
  return (
    <>
      <button
        ...
        className={
          triggerClassName ??
          "inline-flex items-center gap-1 bg-indigo-600 text-white text-sm font-medium px-3 py-2 rounded-lg hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed"
        }
      >
        {triggerLabel ?? "+ Manual refund"}
      </button>
      <Modal
        open={open}
        onClose={() => {
        ...
        <form onSubmit={submit} className="space-y-3">
          {isLocked && order ? (
            <div className="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
              ...
              <div className="text-xs text-slate-500">
                {order.customer_name || "—"} · {order.currency}{" "}
                {order.total_price}
              </div>
            </div>
          ) : (
                  ...
                  <table className="min-w-[680px] text-sm">
                    <thead className="bg-slate-50 text-xs text-slate-600 uppercase">
                      <tr>
                        ...
                        <th className="px-2 py-2 text-right w-20">Qty avail</th>
                          ...
                          <td className="px-2 py-1 text-right">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const subtotal = (q * Number(li.price)).toFixed(
                                  2,
                                );
                                setLines((arr) => {
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                ...
                <label className="text-sm">
                  ...
                  >
                    ...
                    <option value="estebdal">Estebdal (Egypt exchange)</option>
          ...
          <div className="flex flex-col-reverse gap-2 pt-1 sm:flex-row sm:justify-end">
            ...
            <button
              type="submit"
              disabled={
                submitting || !order || orderIneligibleReason(order) !== null
              }
